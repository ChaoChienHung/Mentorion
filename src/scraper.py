...
class Scraper:
    ...
    Members:
    - rate_limiter (RateLimiter): Rate limiter to control API request frequency.
    ...
    def __init__(self, base_urls: List[str] = None, requests_per_minute: int = 60):
        self.rate_limiter = RateLimiter(requests_per_minute=requests_per_minute)  # Initialize the maximum number of requests per minute
